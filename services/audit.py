...
def user_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        # Le statut de restriction est revérifié en base à CHAQUE requête, pas
        # seulement à la connexion : un compte no_login/banned décidé par un
        # admin coupe l'accès immédiatement, même si une session (JWT) encore
        # valide 7 jours existe déjà dans le navigateur du marchand.
        ...
        if is_login_blocked(user):
            logger.info(f"[user_required] accès refusé (compte restreint) user={user['id']}")
            resp = make_response(redirect(url_for('public.login_page', blocked=1)))
...
def admin_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if is_login_blocked(user):
            logger.warning(f"[admin_required] accès refusé (compte restreint) user={user['id']}")
